...
public class ExpressionGenerator extends FilterBaseVisitor<Expression<?>> {
  ...
  public Expression<?> visitPrefix(PrefixContext ctx) {
    ...
    switch (op) {
      ...
      case IS_EMPTY:
        ...
        if (Collection.class.isAssignableFrom(left.getJavaType())) {
  ...
  private Expression<?> visitComparison(InfixContext ctx, InfixOperation op) {
    ...
    if (ctx.left instanceof InputContext && ctx.right instanceof InputContext) {
      ...
      left = visit(ctx.left);
      expectedInputTypes.put(ctx.right, left.getJavaType());
      right = visit(ctx.right);
    }

    else if (ctx.left instanceof InputContext) {
      right = visit(ctx.right);
      expectedInputTypes.put(ctx.left, right.getJavaType());
      left = visit(ctx.left);
    }
  ...
  private Expression<?> visitIn(InfixContext ctx) {
    ...
    for (PredicateContext argument : ctx.arguments) {
      ...
      if (argument instanceof InputContext) {
        expectedInputTypes.put(argument, left.getJavaType());
  ...
  public Expression<?> visitFunction(FunctionContext ctx) {
    ...
    switch (ctx.ID().getText().toLowerCase()) {
      ...
      case "concat":
        List<Expression<String>> arguments = new ArrayList<Expression<String>>();
        for (int i = 0; i < ctx.arguments.size(); i++) {
          arguments.add(getFunctionArgument(ctx, i, String.class));
        }
        return FilterUtils.merge(criteriaBuilder::concat, arguments);
  ...
  @SuppressWarnings("unchecked")
  private <T> Expression<T> getFunctionArgument(
      FunctionContext ctx,
      int index,
  ...
  public Expression<?> visitInput(InputContext ctx) {
    ...
    Object value = StringConverter.convert(ctx.getText(), expectedInputTypes.get(ctx));

    if (value == null) {
      throw new InternalFilterException(
          "The input '" + StringConverter.cleanStringInput(ctx.getText())
              + "' could not be converted to " + expectedInputTypes.get(ctx));
    }
